...
// http://www.enchantedage.com/xna-profiler
namespace KiloWatt.Runtime.Support
{
  public class Profile
  {
    ...
    internal static void Dump(bool andReset)
    {
      Trace.WriteLine("--------------- Profile dump: ---------------");
      Trace.WriteLine("Profiler, ms total, count, ms max");
      Trace.WriteLine("      Histogram (<0.1, <1.0, <10.0, <100.0, ++ ms)");
      double freq = (double)Stopwatch.Frequency;
      foreach (Profile p in profiles_.Values)
      {
        Trace.WriteLine(String.Format("{0}, {1}, {2}, {3}", p.name_, 1000.0 * p.time_ / freq, p.count_,
            1000.0 * p.maximum_ / freq));
        Trace.WriteLine(String.Format("      {0} {1} {2} {3} {4}", p.bins_[0], p.bins_[1], p.bins_[2], p.bins_[3], p.bins_[4]));
        if (andReset) p.Reset();
      }
      Trace.WriteLine("--------------- End profile ---------------");
    }
  }
}
